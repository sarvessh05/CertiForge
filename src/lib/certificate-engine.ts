...
export async function generateCertificates(options: GenerateOptions): Promise<Blob> {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    try {
      ...
      const errorName = row[mapping["name"] || ""] || `certificate_${i + 1}`;
      const safeName = errorName.replace(/[^a-zA-Z0-9_\- ]/g, "").trim();
      zip.file(`${safeName}_ERROR.txt`, `Failed to generate certificate: ${error}`);
      onProgress(i + 1);
    }
  }
